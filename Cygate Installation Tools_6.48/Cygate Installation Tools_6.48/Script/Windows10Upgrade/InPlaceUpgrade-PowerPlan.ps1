...
# Prepare for Logging
Write-Log " "
Write-Log "Setting Powerplan"

$OSDChangePowerPlan = $tsenv.Value("OSDChangePowerPlan")
$OSDRevertPowerPlan = $tsenv.Value("OSDRevertPowerPlan")
$OSDCurrentPowerPlan = $tsenv.Value("OSDCurrentPowerPlan")

# Change PowerPlan during upgrade
$HighPerf = "8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c"
try
{
	$OSDCurrentPowerPlan = $(powercfg -getactivescheme).split()[3]
	Write-Log "Current Powerplan is $OSDCurrentPowerPlan"
}
catch {}

try
{
	if (($OSDCurrentPowerPlan -ne $HighPerf) -and ($OSDRevertPowerPlan -ne $true))
	{
		Write-Log "Current Powerplan is different from high performance, should be changed back after upgrade"
		Write-Log "Setting active powerplan to high performance"
		powercfg -setactive $HighPerf
		# Lid closing options = Do Nothing
		powercfg -setacvalueindex $HighPerf 4f971e89-eebd-4455-a8de-9e59040e7347 5ca83367-6e45-459f-a27b-476b1d01c936 0
		$tsenv.Value("OSDCurrentPowerPlan") = $OSDChangePowerPlan
		$tsenv.Value("OSDRevertPowerPlan") = $true
		Exit 0
	}
}
catch {}

# Revert back to original PowerPlan after upgrade
try
{
	if ($OSDChangePowerPlan -eq $false -and $OSDRevertPowerPlan -eq $true)
	{
		Write-Log "Reverting back to original PowerPlan: $OSDCurrentPowerPlan"
		
		# Lid closing options = Sleep
		powercfg -setacvalueindex $HighPerf 4f971e89-eebd-4455-a8de-9e59040e7347 5ca83367-6e45-459f-a27b-476b1d01c936 1
		
		# Revert powerplan to original state
		$OSDCurrentPowerPlan = $tsenv.Value("OSDCurrentPowerPlan")
		powercfg -setactive $OSDCurrentPowerPlan
	}
}
catch {}