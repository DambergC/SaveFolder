...
# Prepare for Logging
Write-Log " "
Write-Log "Disable Bitlocker"

# Bitlocker Status
try
{
	$BitlockerStatus = Get-CimInstance -Namespace "root/CIMV2/Security/MicrosoftVolumeEncryption" -ClassName Win32_EncryptableVolume -Filter "DriveLetter = 'C:'"
	$ConversionStatus = $($BitlockerStatus.ConversionStatus)
	$IsVolumeInitializedForProtection = $($BitlockerStatus.IsVolumeInitializedForProtection)
	$EncryptionMethod = $($BitlockerStatus.EncryptionMethod)
	$ProtectionStatus = $($BitlockerStatus.ProtectionStatus)
	
	Write-Log "Bitlocker Initialized for Protection: $IsVolumeInitializedForProtection"
	Write-Log "Bitlocker Protection Status:          $ProtectionStatus"
	
	switch ($ConversionStatus)
	{
		0 { $result = 'FullyDecrypted' }
		1 { $result = 'FullyEncrypted' }
		2 { $result = 'EncryptionInProgress' }
		3 { $result = 'DecryptionInProgress' }
		4 { $result = 'EncryptionPaused' }
		5 { $result = 'DecryptionPaused' }
	}
	Write-Log "Bitlocker Conversion Status:          $ConversionStatus, $result"
	
	switch ($EncryptionMethod)
	{
		0 { $result = 'The volume is not encrypted' }
		1 { $result = 'AES_128_WITH_DIFFUSER' }
		2 { $result = 'AES_256_WITH_DIFFUSER' }
		3 { $result = 'AES_128' }
		4 { $result = 'AES_256' }
		5 { $result = 'HARDWARE_ENCRYPTION' }
		6 { $result = 'XTS_AES_128' }
		7 { $result = 'XTS_AES_256' }
	}
	Write-Log "Bitlocker Encryption Method:          $EncryptionMethod, $result"
}
catch { Write-Log "Failed to get Bitlocker status" }
...
if ($ProtectionStatus -eq 1)
{
	try
	{
		$Process = Start-Process -FilePath "manage-bde.exe" -ArgumentList "-protectors -disable C: -RC 0" -Wait -PassThru -NoNewWindow -ErrorAction Stop
		$ErrorCode = $Process.ExitCode
		Write-Log "Encryption disabled with exit code: $ErrorCode"
	}
	catch { Write-Log "Error: Failed to disable encryption" }
}
else { Write-Log "Encryption is already disabled" }

# Adding OSDBitlockerDetected.txt to windir to re-enable bitlocker after upgrade - even if fails
if (($EncryptionMethod -ne 0) -or ($IsVolumeInitializedForProtection -eq $true))
{
	Write-Log "Disk is initialized for encryption"
	if (!(Test-Path -Path "$env:windir\OSDBitlockerDetected.txt"))
	{
		try
		{
			"True" | Out-File -FilePath "$env:windir\OSDBitlockerDetected.txt" -Force -ErrorAction Stop
			Write-Log "Creating OSDBitlockerDetected.txt in $env:windir"
		}
		catch { Write-Log "Error: Failed to create $env:windir\OSDBitlockerDetected.txt" }
	}
	else { Write-Log "$env:windir\OSDBitlockerDetected.txt already in place - skip copying" }
	
	# Setting TS variable
	try
	{
		$tsenv.Value("BitlockerDetected") = $true
		Write-Log "Setting BitlockerDetected to: $true"
	}
	catch { Write-Log "Error: Failed setting BitlockerDetected variable" }
}
else { Write-Log "Disk is not initialized for encryption - will not disable" }
