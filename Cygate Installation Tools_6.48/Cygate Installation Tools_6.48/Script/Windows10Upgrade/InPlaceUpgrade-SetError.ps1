...
# Prepare for Logging
Write-Log " "
Write-Log "Setting Error Code"

if ($($tsenv.Value("OSDWindowsUpgrade")) -eq $false)
{
	Write-Log "OS upgrade considered false - check reason for error in WindowsUpgrade.log, settings error 8021"
	exit 8021
}
else
{
	Write-Log "OS upgrade considered true (upgrade should have occured) ended with error, setting error 8023"
	Exit 8023
}
