...
# Prepare for Logging
Write-Log " "
Write-Log "Installing Background"

if (Get-Process msiexec -ErrorAction silentlycontinue) {
	try
	{
		Write-Log "Kill all running msiexec processes"
		Start-Process -FilePath "taskkill.exe" -ArgumentList "/IM msiexec.exe /F /T" -Wait -PassThru -NoNewWindow -ErrorAction SilentlyContinue
	}
	catch { Write-Log "Could not kill msiexec.exe" }
}
else { Write-Log "No running msiexec processes to shut down" }

Stop-Service -Name "UPGBackgroundService" -Force -ErrorAction SilentlyContinue

try
{
	Write-Log "Start uninstalling if already installed"
	$Process = Start-Process -FilePath "msiexec.exe" -ArgumentList "/x `"{D84B4FD0-EB52-42C4-94AB-EF49F03D90A4}`" /qn /norestart"
	$ErrorCode = $Process.ExitCode
	Write-Log "Uninstall background with exit code: $ErrorCode"
}
catch { Write-Log "Could not uninstall background" }

try
{
	$Process = Start-Process -FilePath "msiexec.exe" -ArgumentList "/i `"$PSScriptRoot\UPGBackground.msi`" /qn /norestart" -Wait -NoNewWindow -PassThru -ErrorAction Stop
	$ErrorCode = $Process.ExitCode
	Write-Log "Install background with exit code: $ErrorCode"
}
catch { Write-Log "Error: Failed to install background" }

# Disable Lock screen if enabled
Write-Log "Check if Lock screen is enabled - will disable tempoarily to allow upgrade background to stay ontop"
try { $LockScreen = Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Personalization" -Name "NoLockScreen" -ErrorAction Stop } catch { $LockScreen = $null }
$tsenv.Value("OSDLockScreen") = $LockScreen
if (($LockScreen -eq $null) -or ($LockScreen -eq "0")) { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Personalization" -Name "NoLockScreen" -Value "1" -Type Dword -Force -ErrorAction SilentlyContinue }
if ($LockScreen -ne $null) { Write-Log "LockScreen is configured in registry: $LockScreen" } else { Write-Log "LockScreen is not configured in registry" }

# Disable screen saver
Write-Log "Check if screen saver is already disabled (or configured for all users)"
try { $ScreenSaveActive = Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" -Name "ScreenSaveActive" -ErrorAction Stop } catch { $ScreenSaveActive = $null; Write-Log "ScreenSaveActive not configured for all users (null)" }
try { $InactivityTimeoutSecs = Get-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System" -Name "InactivityTimeoutSecs" -ErrorAction Stop } catch { $InactivityTimeoutSecs = $null; Write-Log "InactivityTimeoutSecs not configured for all users (null)" }
$tsenv.Value("ScreenSaveActive") = $ScreenSaveActive
$tsenv.Value("InactivityTimeoutSecs") = $InactivityTimeoutSecs
try { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" -Name "ScreenSaveActive" -Value "0" -Type Dword -Force -ErrorAction SilentlyContinue; Write-Log "Setting ScreenSaveActive to 0" } catch { Write-Log "Error setting ScreenSaveActive" }
try { Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System" -Name "InactivityTimeoutSecs" -Value "0" -Type Dword -Force -ErrorAction SilentlyContinue; Write-Log "Setting InactivityTimeoutSecs to 0" } catch { Write-Log "Error setting InactivityTimeoutSecs" }
