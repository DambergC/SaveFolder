...
# Prepare for Logging
Write-Log " "
Write-Log "Checking Driver Folder"

# If driver folder not in place during upgrade, setup will crash with error code. Virtual machines does not stage drivers and therefore this folder is not created.
if (!(Test-Path "$env:SystemDrive\Drivers"))
{
	New-Item -Path "$env:SystemDrive\Drivers" -ItemType dir -Force
	Write-Log "Driver Folder not found, creating"
}
else { Write-Log "Driver Folder already in place" }