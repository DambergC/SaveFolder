...
$FileCount = (Get-ChildItem -Path "$env:SystemDrive\Drivers\*.*" -Recurse -Force).Count
if (($FileCount -lt 2) -and ($Size -lt 50) -and (Test-Path "$env:SystemDrive\Drivers\DriverOK.txt")) { Write-Log "Driver not downloaded properly, deleting DriverOK.txt and retry"; Remove-Item -Path "$env:SystemDrive\Drivers\DriverOK.txt" -Force -ErrorAction SilentlyContinue }
