...
# Prepare for Logging
Write-Log " "
Write-Log "SetupDiag check"

$ParentDirectory = (Get-Item $PSScriptRoot).parent.parent.FullName

# Run SetupDiag
if (Test-Path "$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log") { Remove-Item -Path "$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log" -Force }
Write-Log "Running SetupDiag with following arguments: $ParentDirectory\Tools\Microsoft\SetupDiag.exe /Output:`"$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log`" /Verbose"
$Process = Start-Process -FilePath "$ParentDirectory\Tools\Microsoft\SetupDiag.exe" -ArgumentList "/Output:`"$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log`" /Verbose" -Wait -PassThru -NoNewWindow -ErrorAction SilentlyContinue
$ErrorCode = $Process.ExitCode
Write-Log "SetupDiag check done with exit code: $ErrorCode"

# Get information from upgrade logs
Write-Log "Checking upgrade logs..."
	...
	foreach ($line in $LogContent)
	{
		...
		if (($line -like "*<DriverPackage Inf*") -and ($line -like '*BlockMigration="True"*'))
		{
			try
			{
				...
				$ErrorArray += $DriverName
				Write-Log "Blocking driver found: $ProgramName"
				Write-GlobalLog "Error: Blocking driver found: $ProgramName"
			}
			catch { }
		}
	}
...
try
{
	if ($ErrorDriverArray.Count -gt 0) { $tsenv.Value("ErrorDriverArray") = $ErrorDriverArray }
}
catch { }