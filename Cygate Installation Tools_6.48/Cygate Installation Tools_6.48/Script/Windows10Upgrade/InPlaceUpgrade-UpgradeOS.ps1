...
if (Test-Path "$ISO")
{
	try
	{
		if ($ISO -ne $null)
		{
			...
			if (($DriveLetter -ne $null) -and ($MountResult -eq $true))
			{
				...
				else { Write-Log "Unable to find $DriveLetter\Sources\install.wim"; $DriveLetter = $null; $ISOMounted = $false }
			}
			if (($null -eq $DriveLetter) -and ($ISOMounted -eq $false))
			{
	...
	if ($DriveLetter -ne $null)
	{
		try
		{
			...
			# Customize SetupRollbackTemplate.cmd
			Write-Log "Customize SetupRollbackTemplate.cmd in case if failure. Add fx function to not set agent in provision mode"
			if (Test-Path -Path "$env:windir\CCM\SetupRollbackTemplate.cmd")
			{
				if (!((Get-Content -Path "$env:windir\CCM\SetupRollbackTemplate.cmd" -ErrorAction SilentlyContinue) -contains "Trigger Hardware Inventory"))
				{
					$Text = "REM Adding Custom Items HERE`r`n" + "echo %DATE%-%TIME% Adding Cygate Additional Items >> %WINDIR%\setuprollback.log`r`n" +
					"echo %DATE%-%TIME% Run Command to remove from provisioning mode >> %WINDIR%\setuprollback.log`r`n" + "WMIC /namespace:\\root\ccm path sms_client CALL SetClientProvisioningMode `"False`" /NOINTERACTIVE`r`n" +
					"echo %DATE%-%TIME% Removing from provisioning mode Complete >> %WINDIR%\setuprollback.log`r`n" + " `r`n" +
					"REM Setting WaaS Key Registry Key, requires %SMSTS_Build% variable in TS set.`r`n" + "echo %DATE%-%TIME% Setting WaaS_Stage RegKey to IPU_RollBack >> %WINDIR%\setuprollback.log`r`n" +
					"REG ADD `"HKEY_LOCAL_MACHINE\SOFTWARE\WaaS\%SMSTS_Build%`" /V WaaS_Stage /T REG_SZ /D `"IPU_RollBack`" /F`r`n" + " `r`n" +
					"REM Trigger Hardware Inventory:" + "echo %DATE%-%TIME% Trigger Hardware Inventory >> %WINDIR%\setuprollback.log`r`n" +
					"WMIC /namespace:\\root\ccm path sms_client CALL TriggerSchedule `"{ 00000000 - 0000 - 0000 - 0000 - 000000000001 }`" /NOINTERACTIVE`r`n" +
					"echo %DATE%-%TIME% Exiting setuprollback.cmd after additional custom commands >> %WINDIR%\setuprollback.log`r`n" + " `r`n" +
					"set SCCMClientPath="
					
					((Get-Content -path "$env:windir\CCM\SetupRollbackTemplate.cmd" -Raw) -replace 'set SCCMClientPath=', $Text) | Set-Content -Path "$env:windir\CCM\SetupRollbackTemplate.cmd"
				}
				else { Write-Log "$env:windir\CCM\SetupRollbackTemplate.cmd already customized - no change applied" }
			} else { Write-Log "Could not find $env:windir\CCM\SetupRollbackTemplate.cmd" }
			
			# Customize SetupCompleteTemplate.cmd
			Write-Log "Customize SetupCompleteTemplate.cmd to add hw inventory after upgrade"
			if (Test-Path -Path "$env:windir\CCM\SetupCompleteTemplate.cmd")
			{
				if (!((Get-Content -Path "$env:windir\CCM\SetupCompleteTemplate.cmd" -ErrorAction SilentlyContinue) -contains "Trigger Hardware Inventory"))
				{
					$Text = "REM Adding Custom Items HERE`r`n" + "echo %DATE%-%TIME% Adding Cygate Additional Items >> %WINDIR%\setupcomplete.log`r`n" +
					"REM Trigger Hardware Inventory:" + "echo %DATE%-%TIME% Trigger Hardware Inventory >> %WINDIR%\setupcomplete.log`r`n" +
					"WMIC /namespace:\\root\ccm path sms_client CALL TriggerSchedule `"{ 00000000 - 0000 - 0000 - 0000 - 000000000001 }`" /NOINTERACTIVE`r`n" +
					"echo %DATE%-%TIME% Exiting SetupCompleteTemplate.cmd after additional custom commands >> %WINDIR%\setupcomplete.log`r`n" + " `r`n" +
					"set SCCMClientPath="
					
					((Get-Content -path "$env:windir\CCM\SetupCompleteTemplate.cmd" -Raw) -replace 'set SCCMClientPath=', $Text) | Set-Content -Path "$env:windir\CCM\SetupCompleteTemplate.cmd"
				}
				else { Write-Log "$env:windir\CCM\SetupCompleteTemplate.cmd already customized - no change applied" }
			}
			else { Write-Log "Could not find $env:windir\CCM\SetupCompleteTemplate.cmd" }
			...
			if ($tsenv.Value("OSDRequireDrivers") -ne $true) { Write-Log "Drivers not required, will not set staging path variable" }
			else
			{
				Write-Log "Drivers required - will set staging path variable"
			...
			if ($ErrorCode -ne 0)
			{
				Write-Log "Warning: exit code not 0 ($ErrorCode) will try gather diagnostics files and copy to log share"
				
				Write-Log "Check status of CCmExec Service"
				if ((Get-Service -Name ccmexec | Select-Object *).StartType -like "*Automatic*") { Write-Log "CCMExec service is OK - Automatic start" }
				else { Set-Service -Name ccmexec -StartupType Automatic -ErrorAction SilentlyContinue }
			}
			
			if ($ErrorCode -ne 0)
			{
				Write-Log "Checking for (and removing) blocking drivers"
				...
				if (Test-Path -Path "$LogFile")
				{
					...
					{
						if (($line -like "*<DriverPackage Inf*") -and ($line -like '*BlockMigration="True"*'))
						{
							try
							{
								$DriverName = $line.Substring($line.IndexOf("=") + 2)
								$DriverName = $DriverName.Substring(0, $DriverName.IndexOf('BlockMigration=') - 2)
								Write-Log "Blocking driver found: $DriverName - will try uninstalling"
								try { pnputil -f -d $env:windir\INF\$DriverName; Write-Log "Driver $DriverName uninstalled" } catch { Write-Log "Failed to uninstall $DriverName" }
							}
							catch { }
						}
					}
				...
				# Retry Upgrade
				Write-Log "Start Upgrading OS using OSDUpgradeOS.exe (retry after uninstalling blocking drivers)"
				$Process = Start-Process -FilePath "$env:windir\CCM\OSDUpgradeOS.exe" -Wait -PassThru -NoNewWindow -ErrorAction SilentlyContinue
				$ErrorCode = $Process.ExitCode
				Write-Log "Setup ended with exit code: $ErrorCode (2nd try)"
				$tsenv.Value("ReturnCode") = $ErrorCode
			}
