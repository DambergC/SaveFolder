...
# Prepare for Logging
Write-Log " "
Write-Log "Get Return Code from Registry"

if ($($tsenv.Value("ForceUpgrade")) -eq $true) { Write-Log "ForceUpgrade is true (No user logged on) - will not check for returncode in registry" }
else
{
	Write-Log "ForceUpgrade is false (User logged on or TS variable to force upgrade is FALSE) - will check for returncode in registry"
	# Get ReturnCode from Registry
	try
	{
		$ReturnCode = (Get-ItemProperty -Path "HKLM:\Software\Windows10Upgrade" -Name "ReturnCode" -ErrorAction Stop).ReturnCode
		if ($ReturnCode -eq $null)
		{
			Write-Log "Could not find a return code in registry"
		}
		else
		{
			Write-Log "Getting Return Code from Registry: $ReturnCode"
			$tsenv.Value("returncode") = $ReturnCode
		}
	}
	catch { Write-Log "Error Getting Return Code from Registry, setting 8022 as default (stop upgrade because if missing information)"; $tsenv.Value("returncode") = 8022 }
	
	try
	{
		$OSDChangePowerPlan = (Get-ItemProperty -Path "HKLM:\Software\Windows10Upgrade" -Name "OSDChangePowerPlan" -ErrorAction Stop).OSDChangePowerPlan
		if ($OSDChangePowerPlan -eq $null)
		{
			Write-Log "Could not find OSDChangePowerPlan in registry"
		}
		else
		{
			Write-Log "Getting OSDChangePowerPlan from Registry: $OSDChangePowerPlan"
			$tsenv.Value("OSDChangePowerPlan") = $OSDChangePowerPlan
		}
	}
	catch { Write-Log "Not able to get OSDChangePowerPlan from Registry" }
	
	try
	{
		$gotoEnd = (Get-ItemProperty -Path "HKLM:\Software\Windows10Upgrade" -Name "gotoEnd" -ErrorAction Stop).gotoEnd
		if ($gotoEnd -eq $null)
		{
			Write-Log "Could not find gotoEnd in registry"
		}
		else
		{
			Write-Log "Getting gotoEnd from Registry: $gotoEnd"
			$tsenv.Value("gotoEnd") = $gotoEnd
		}
	}
	catch { Write-Log "Not able to get gotoEnd from Registry" }
}