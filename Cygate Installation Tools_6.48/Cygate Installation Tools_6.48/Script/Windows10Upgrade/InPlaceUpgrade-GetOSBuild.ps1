...
if (Test-Path "$OSDOSImagePath")
{
	try
	{
		...
		if ($ISO -ne $null)
		{
			...
			if (Test-Path "$DriveLetter\Sources\install.wim") { Write-Log "$DriveLetter\Sources\install.wim found assuming mounted ISO" } else { $DriveLetter = $null; Write-Log "$DriveLetter\Sources\install.wim not found, failed mounting ISO" }
		}
		if (($null -eq $DriveLetter) -and ($null -ne $ISO))
		{
	...
	if ($DriveLetter -ne $null)
	{
		...
		# Get enablement packages to distinguise between 190xx versions (All new Windows version reports 19041 as core)
		# https://support.microsoft.com/en-us/topic/version-and-build-number-are-reported-incorrectly-after-you-build-a-new-windows-image-file-0141c14e-b3b6-e4ab-88bb-6e3ba0d96b14
		# https://superuser.com/questions/1693090/downloading-windows-10-21h2-wrong-build
		try
		{
			Write-Log "Getting enablement packages to determine build version (19041, 19042, 19043, 19044, 19045)"
			if (Test-Path "$env:SystemDrive\MountTemp") { Remove-Item -Path "$env:SystemDrive\MountTemp" -Recurse -Force -ErrorAction SilentlyContinue }
			New-Item -Path "$env:SystemDrive\MountTemp" -ItemType Dir -Force -ErrorAction SilentlyContinue
			Dism /Mount-Wim /WimFile:"$DriveLetter\Sources\install.wim" /index:1 /MountDir:"$env:SystemDrive\MountTemp" /ReadOnly
			$Packages = (dism /Image:"$env:SystemDrive\MountTemp" /Get-Packages)
			if ($Packages -like "*KB4562830*") { $Version = "19042"; Write-Log "Enablement package for 20H2 found, setting version: $Version" }
			if ($Packages -like "*KB5000736*") { $Version = "19043"; Write-Log "Enablement package for 21H1 found, setting version: $Version" }
			if ($Packages -like "*KB5003791*") { $Version = "19044"; Write-Log "Enablement package for 21H2 found, setting version: $Version" }
			if ($Packages -like "*KB5015684*") { $Version = "19045"; Write-Log "Enablement package for 22H2 found, setting version: $Version" }
			dism /Unmount-Wim /MountDir:"$env:SystemDrive\MountTemp" /discard
			Remove-Item -Path "$env:SystemDrive\MountTemp" -Recurse -Force -ErrorAction SilentlyContinue
		}
		catch { Write-Log "Error: Failed to get enablement packages from wim file: $DriveLetter\Sources\install.wim" }
