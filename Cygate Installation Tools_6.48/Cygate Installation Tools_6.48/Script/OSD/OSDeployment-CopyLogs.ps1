...
#	if ($ErrorCode -ne 0){ exit }
	...
	try
	{
		...
		$Credential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $UsernameFull, $userPass
		$DriveError = New-PSDrive -Name T -PSProvider FileSystem -Root $Logshare -Credential $Credential -ErrorAction Stop
		if (Test-Path -Path "T:") { Write-Log  "Mapped T: using PSDrive method successfully" } else { Write-Log "Error: Failed to map T: using PSDrive method" }
	}
	catch [System.Exception]{
		$Message = $_.Exception.Message
...
try
{
	$Process = Start-Process -FilePath "NET.exe" -ArgumentList "USE T: /DELETE /Y" -WindowStyle Hidden -Wait -PassThru -ErrorAction SilentlyContinue
