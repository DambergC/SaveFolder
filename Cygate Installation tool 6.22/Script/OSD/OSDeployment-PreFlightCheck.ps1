...
foreach ($LogServer in $LogServers)
{
	...
	try
	{
		...
		if ($ErrorCode -ne 0) { Write-Log "Log Share - Error: Failed mapping Log Share using NET USE" }	else { $ErrorMapping = $false; Write-Log "Log Share - Mapped Log Share successfully" }
	}
...
if (($VM -ne $true) -or ($Manufacturer -like "*VMware*")) # VMware requires drivers
{
	if ($tsenv.Value("OSDUseCheckDriverService") -eq $true)
	{
		Write-Log "Driver - Driver method: Check Driver Service"
		if ($DriverFound -eq $true)
		{
			Write-Log "Driver - Drivers for model found, PackageID for driver: $PackageID"
		}
