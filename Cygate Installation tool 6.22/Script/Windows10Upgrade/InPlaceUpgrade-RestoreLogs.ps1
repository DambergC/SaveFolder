...
# Prepare for Logging
Write-Log " "
Write-Log "Restore Log files"

if (!(Test-Path "$env:windir\Logs\Software")) { New-Item -Path "$env:windir\Logs\Software" -ItemType dir -Force }
	
try
{
	if (Test-Path "$env:SystemDrive\Windows.old\Windows\Logs\Software")
		{
			Write-Log "Restored Log files from Windows.old to Windows"
			Start-Sleep -Seconds 2
			Copy-Item -Path "$env:SystemDrive\Windows.old\Windows\Logs\Software\*.*" -Destination "$env:windir\Logs\Software" -Force
		}
}
catch { Write-Log "Error: Failed to restore Log files from Windows.old to Windows" }
