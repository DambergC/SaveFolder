...
if ($tsenv.Value("OSDDriverPackageID") -eq $null) { Write-Log "Error: Driver variable not populated, skipping download - driver for PC not found"; exit 0 }
...
if (Test-Path "$env:SystemDrive\Drivers")
{
	try
	{
		$Process = Start-Process -FilePath "dism.exe" -ArgumentList "/image:%OSDisk%\ /Add-Driver /Driver:`"$env:SystemDrive\Drivers`" /recurse" -Wait -NoNewWindow -PassThru -ErrorAction SilentlyContinue
		$ErrorCode = $Process.ExitCode
...
Write-Log "$env:SystemDrive\Drivers removed"
