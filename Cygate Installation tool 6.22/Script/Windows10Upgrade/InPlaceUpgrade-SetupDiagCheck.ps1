...
Write-Log "Running SetupDiag with following arguments: $ParentDirectory\Tools\Microsoft\SetupDiag.exe /Output:`"$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log`" /LogsPath:`"$env:windir\Logs\Software`" /Verbose"
$Process = Start-Process -FilePath "$ParentDirectory\Tools\Microsoft\SetupDiag.exe" -ArgumentList "/Output:`"$env:windir\Logs\Software\SetupDiag_WindowsUpgrade.log`" /LogsPath:`"$env:windir\Logs\Software`" /Verbose" -Wait -PassThru -NoNewWindow -ErrorAction SilentlyContinue
$ErrorCode = $Process.ExitCode
