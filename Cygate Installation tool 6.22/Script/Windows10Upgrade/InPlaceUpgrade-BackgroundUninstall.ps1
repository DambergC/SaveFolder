...
# Prepare for Logging
Write-Log " "
Write-Log "Uninstalling Background"

# Enable Lock screen if previously enabled
Write-Log "Check if Lock screen is was enabled before upgrade"
$LockScreen = $tsenv.Value("OSDLockScreen")
if (($LockScreen -ne "0") -and ($LockScreen -ne "1")) { Write-Log "Lock screen TS variable is null - removing value from registry"; try { Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Personalization" -Name "NoLockScreen" -Force -ErrorAction Stop } catch { Write-Log "Could not remove NoLockScreen registry key" } }
if ($LockScreen -eq "0") { Write-Log "Lock screen TS variable is: $LockScreen - setting value to 0"; try { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Personalization" -Name "NoLockScreen" -Value "0" -Type Dword -Force -ErrorAction Stop } catch { Write-Log "Could not set NoLockScreen registry key to value 0" } }

# Enable screensaver if previously enabled
Write-Log "Check if Screen saver is was enabled before upgrade"
$ScreenSaveActive = $tsenv.Value("ScreenSaveActive")
$InactivityTimeoutSecs = $tsenv.Value("InactivityTimeoutSecs")
if (($ScreenSaveActive -ne "0") -and ($ScreenSaveActive -ne "1")) { Write-Log "ScreenSaveActive TS variable is null - removing value from registry"; try { Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" -Name "ScreenSaveActive" -Force -ErrorAction Stop } catch { Write-Log "Could not remove ScreenSaveActive value in registry" } }
else { Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" -Name "ScreenSaveActive" -Value "$ScreenSaveActive" -Type Dword -Force -ErrorAction SilentlyContinue; Write-Log "Setting ScreenSaveActive to $ScreenSaveActive" }
if (($InactivityTimeoutSecs -ne "0") -and ($InactivityTimeoutSecs -ne "1")) { Write-Log "InactivityTimeoutSecs TS variable is null - removing value from registry"; try { Remove-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System" -Name "InactivityTimeoutSecs" -Force -ErrorAction Stop } catch { Write-Log "Could not remove InactivityTimeoutSecs value in registry" } }
else { Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System" -Name "InactivityTimeoutSecs" -Value "$InactivityTimeoutSecs" -Type Dword -Force -ErrorAction SilentlyContinue; Write-Log "Setting InactivityTimeoutSecs to $InactivityTimeoutSecs" }
...
catch { Write-Log "Error: Failed to uninstall background" }
