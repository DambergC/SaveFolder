...
if ($ErrorPermission -eq $true) { $tsenv.Value("OSDWindowsUpgrade") = $false; $tsenv.Value("gotoEnd") = $true; $returnCode = 8022; Write-GlobalLog "rror: Permissions not suffcient on Log Share: $LogShare" }
...
if (($LoggedOnRDPUser -eq $false) -and ($LoggedOnUser -eq $false)) { $returnCode = 8022; Write-Log "Logged on user - No user logged on, setting returncode 8022 to stop upgrade" }
...
if (Test-Path 'C:\$WINDOWS.~BT')
{
	...
	Write-Log "Removing 'C:\SWINDOWS.~BT' folder"
}
